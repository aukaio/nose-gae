...
class NoseGAE(Plugin):
    ...
    def configure(self, options, config):
        ...
        try:
            ...
            # and is used by imports. Because typical usage is to do imports
            # and then WebTest, we need to set these variables upfront,
            # before any imports that check runtime version
            env_dict = os.environ
            gaeconfig, _, _ = dev_appserver.LoadAppConfig(self._path, {})
            version = dev_appserver.GetVersionObject()
            env_dict['SDK_VERSION'] = version['release']
            env_dict['CURRENT_VERSION_ID'] = gaeconfig.version + ".1"
            env_dict['APPLICATION_ID'] = gaeconfig.application
